...
function MarkPaths({ stroke = INK, sw = 1.05, fill = INK, echoes = true, perfs = true, axis = true, spiral = true }) {
  const { cx, cy } = G;
  return (
    <g>
      {echoes && (
      ...
      <circle cx={cx} cy={cy} r={G.rDot} fill={fill} />
      {perfs && (
        <g fill={fill}>
          {[
...
function PaperGrain() {
  ...
  return (
    <div style={{
      ...
      backgroundImage: "radial-gradient(rgba(11,11,12,0.025) 1px, transparent 1px)",
  ...
  return (
    <div style={{ width: "100%", height: "100%", background: PAPER, position: "relative", overflow: "hidden" }}>
      ...
      <div style={{ position: "absolute", inset: 0, display: "grid", placeItems: "center" }}>
        <EchoMark size={460} fg={INK} stroke={1.05} />
      </div>
...
function ArtboardManifesto() {
  return (
    <div style={{ width: "100%", height: "100%", background: INK, color: CREAM, position: "relative", overflow: "hidden", padding: "92px 96px", display: "flex", flexDirection: "column", justifyContent: "space-between" }}>
      <FilmGrain />
