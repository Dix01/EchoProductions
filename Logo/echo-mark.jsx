...
// ────────────────────────────────────────────────────────────────────────────
// AI gestures:
//   • lattice — 8 latent-space nodes on a φ-derived ring with hairline edges
//     (a neural graph encircling the lens — the model surrounding the eye)
//   • recursive — a mini copy of the mark inside the pupil (self-reference,
//     the system that contains itself; an AI training on its own output)
//
// Both are tied to scale: they fade at small sizes so the mark stays legible.
...
function EchoMark({
  ...
  const { cx, cy } = G;
  // node ring — 8 latent nodes between iris and echo, on the spoke axes
  const N = 8;
  ...
  return (
    ...
    >
      ...
      {/* recursive mini-mark inside the pupil — self-reference */}
      {recursive && (
        <g transform={`translate(${cx} ${cy}) scale(0.115) translate(${-cx} ${-cy})`} opacity={0.78}>
          <circle cx={cx} cy={cy} r={G.rBarrel} fill="none" stroke={sc} strokeWidth={stroke / 0.115 * 0.85} />
          <g stroke={sc} strokeWidth={stroke / 0.115 * 0.85} fill="none" strokeLinecap="round">
            {APERTURE.blades.map((d, i) => <path key={i} d={d} />)}
          </g>
          <polygon points={APVERT_STR} fill={fc} stroke="none" />
        </g>
      )}

      {/* latent-space lattice — 8 nodes on a φ-derived ring with hairline edges */}
      {lattice && (
        <g>
          {/* edges — adjacent nodes only (clean ring) */}
          <g stroke={sc} strokeWidth={stroke * 0.35} fill="none" opacity={0.55}>
            {nodes.map((n, i) => {
              const m = nodes[(i + 1) % N];
              return <line key={i} x1={n.x} y1={n.y} x2={m.x} y2={m.y} />;
            })}
          </g>
          {/* nodes — Fibonacci-flagged: every 3rd is filled (trained sample) */}
          {nodes.map((n, i) => (
            <circle
              key={i}
              cx={n.x} cy={n.y}
      ...
      {/* central focal dot — only when no recursive mark replaces it */}
      {!recursive && <circle cx={cx} cy={cy} r={G.rDot} fill={fc} />}
